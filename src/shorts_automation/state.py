"""Quản lý trạng thái (state.json) để không tạo trùng đoạn video/audio giữa các lần chạy.

Mỗi cặp (video_path, narration_path) có một "source key" riêng, lưu:
- video_pointer_sec / audio_pointer_sec: mốc thời gian đã dùng tới, đoạn tiếp theo
  luôn bắt đầu từ đây trở đi (đảm bảo không lấy lại đoạn cũ, không lồng nhau).
    ...
    def record_short(
        ...
        *,
        ...
        entry = {
            ...
            "duration": round(video_end - video_start, 3),
            "title": title,
